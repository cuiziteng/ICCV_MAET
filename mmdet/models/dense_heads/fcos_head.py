...
class FCOSHead(AnchorFreeHead):
    ...
    """  # noqa: E501
    # FPN架构中输出len==5的特征
    # the default value of regress_ranges and center_sample_radius are assigner
    # regress_ranges: 按照 scale 将不同大小的 gt bbox 分配给不同的 FPN 层进行预测
    # center sampling (default = False) 如果不开启中心采样，那么 gt bbox 内部的所有点都属于正样本，
    # 一旦开启了中心采样，则需要依据采样半径计算出每个 gt bbox 内部哪些点属于正样本。
    # center_sample_radius: 在某个特定预测层，哪些位置负责预测该 gt bbox
    def __init__(self,
                 num_classes,
                 in_channels,
    ...
    def loss(self,
        ...
        # return the point to from feature scale to image scale
    ...
    def _get_bboxes_single(self,
        ...
        if with_nms:
            det_bboxes, det_labels = multiclass_nms(    # change the nms method to soft-nms method
                mlvl_bboxes,
                mlvl_scores,
                cfg.score_thr,
                cfg.nms,
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        # 组成 nxmx4的矩阵，表示每个点距离所有 gt bbox 4条边的距离值
